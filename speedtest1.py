...
class Speedtest(object):
    ...
    def get_best_server(self, servers=None, closest_limit=15, no_geo_filter=False):
        """Perform a speedtest.net "ping" to determine which speedtest.net
        server has the lowest latency
        
        Args:
            servers: List of specific servers to test (optional)
            closest_limit: Number of closest servers to test (when geo filtering enabled)
            no_geo_filter: If True, test ALL available servers instead of just closest ones
        """

        if not servers:
            if no_geo_filter:
                # Test ALL servers - flatten the servers dictionary
                servers = []
                if not self.servers:
                    self.get_servers()  # Get all available servers
                
                # Flatten all servers from all distances into one list
                for distance_group in self.servers.values():
                    servers.extend(distance_group)
                
                printer('Testing ALL %d available servers for latency...' % len(servers), 
                       debug=True)
            else:
                # Use geographic filtering (original behavior)
                if not self.closest:
                    servers = self.get_closest_servers(limit=closest_limit)
                servers = self.closest
                
                printer('Testing %d closest servers for latency...' % len(servers), 
                       debug=True)
        ...
        for idx, server in enumerate(servers):
            printer('Testing server %d/%d: %s (%s)' % 
                   (idx + 1, total_servers, server['sponsor'], server['name']), 
                   debug=True)
            ...
            # Fixed the calculation bug - should divide by 3, not 6
            avg = round((sum(cum) / 3) * 1000.0, 3)  # Average of 3 tests in milliseconds
            results[avg] = server
    ...
    def show_server_count(self):
        """Display information about available servers"""
        if not self.servers:
            self.get_servers()
        
        total_servers = sum(len(server_list) for server_list in self.servers.values())
        distances = sorted(self.servers.keys())
        
        printer('Total available servers: %d' % total_servers, debug=True)
        printer('Distance range: %.2f km to %.2f km' % 
               (distances[0] if distances else 0, 
                distances[-1] if distances else 0), debug=True)
...
def shell():
    ...
    if not args.mini:
        ...
        elif args.no_geo_filter:
            printer('Testing ALL servers for best latency (this may take a while)...', quiet)
        else:
            printer('Selecting best server based on ping...', quiet)
            
        # Updated call to include new parameters
        speedtest.get_best_server(
            closest_limit=getattr(args, 'closest_servers', 15),
            no_geo_filter=args.no_geo_filter
        )
    elif args.mini:
...
if __name__ == '__main__':
    main()-download', dest='download', default=True,
    ...
    parser.add_argument('--no-pre-allocate', dest='pre_allocate',
                        ...
                        help='Do not pre allocate upload data. Pre allocation '
                             ...
                             'MemoryError')
    parser.add_argument('--closest-servers', type=PARSER_TYPE_INT, default=15,
                        help='Number of closest servers to test for best latency when '
                             'geographic filtering is enabled. Default 15. Ignored when '
